<template>
  <Dialog :title="__('Test routing')" :open="open" @update:open="emit('update:open', $event)">
    <template #default>
      <div class="flex flex-col gap-4">
        ...
        <template v-else>
          ...
          <FormControl :label="__('Ticket Type')" type="text" v-model="fields.ticket_type" />
          <FormControl
        ...
        <template v-if="results !== null">
          ...
          <div
            ...
            class="rounded-md bg-surface-green-1 border border-outline-green-2 px-4 py-3"
          >
            <span class="text-p-sm text-ink-gray-6">{{ __("Would route to") }}</span>
            <span class="ms-2 text-base-semibold text-ink-gray-8">{{
          ...
          >
            <span class="text-p-sm text-ink-gray-6">{{
              __("No policy matched — the ticket would keep its default team.")
            }}</span>
          </div>
          ...
          >
            {{ __("No enabled policies to evaluate.") }}
          </p>

          <!-- Per-policy rows -->
          <div v-else class="flex flex-col gap-1">
            <div
              v-for="(row, idx) in results.results"
              :key="idx"
              class="flex flex-col gap-1 rounded-md border border-outline-gray-1 px-3 py-2"
            >
              <div class="flex items-center justify-between gap-2">
                <span class="text-base-medium text-ink-gray-7">{{ row.team }}</span>
                <!-- matched: first = Winner solid green; later = unreachable subtle gray -->
                <Badge
                  v-if="row.status === 'matched' && idx === firstMatchedIdx"
                  :label="__('Winner')"
                  theme="green"
                  variant="solid"
                />
                ...
                />
                <Badge
                  v-else-if="row.status === 'skipped_empty_condition'"
                  :label="__('Skipped — no condition')"
                  theme="gray"
                  variant="subtle"
                />
                <Badge
                  v-else-if="row.status === 'skipped_team_disabled'"
                  :label="__('Skipped — team disabled')"
                  theme="gray"
                  variant="subtle"
                />
                <Badge
...
// Index of the first matched policy — that row is the live "Winner"
const firstMatchedIdx = computed(() =>
  results.value?.results.findIndex((r) => r.status === "matched") ?? -1
);
...
async function runTest() {
  ...
  try {
    ...
    results.value = await call("axe_helpdesk.api.team_assignment.simulate", payload);
  } catch (e: any) {
