...
function switchToView(view: string): boolean {
  ...
  if (view) {
    // Stale ?view: drop it but keep the filters, unlike a push to the bare
    // route. Keep the hash — it carries the detail pane's tab/anchor state
    // (e.g. #activity) in split view.
    router.replace({
      name: route.name,
